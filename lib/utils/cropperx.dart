...
class _CropperState extends State<Cropper> {
                  ...
                  child: LayoutBuilder(
                    builder: (_, constraint) {
                      return InteractiveViewer(
                        ...
                        minScale: _minScale,
                        maxScale: _minScale < widget.zoomScale
                            ? widget.zoomScale
                            : _minScale,
                      );
                    },
  ...
  double _getCoverRatio(Size outside, Size inside) {
    dev.log(
        "_getCoverRatio ${outside.width}x${outside.height} : ${inside.width}x${inside.height}");
  ...
  void _setInitialScale(BuildContext context, Size parentSize) {
    WidgetsBinding.instance.addPostFrameCallback((_) {
      ...
      if (childSize != Size.zero) {
        final _ratio = _getCoverRatio(parentSize, childSize);
        _transformationController.value =
            Matrix4.identity() * _ratio as Matrix4;
        _minScale = _ratio;
        setState(() {});
        dev.log("_transformationController ${_transformationController.value}");
      }
